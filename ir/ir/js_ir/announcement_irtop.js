...
function drawDataArea(data) {
	...
	var elmDiv = document.getElementById('newsBox');
//	if (null == elmDiv) {
	...
	if (true == elmDiv.hasChildNodes()) {
		for (var i in elmDiv.childNodes) {
			if (null != elmDiv.childNodes[i].nodeName && undefined != elmDiv.childNodes[i].nodeName) {
				if (elmDiv.childNodes[i].nodeName.toUpperCase() == 'article'.toUpperCase()) {
...
function createData(element, item, IsNew) {
	...
	// var elmDd = document.createElement('dd');
		...
		elmDd.appendChild(elmUl);
	// var elmDl = document.createElement('article');
	// elmDl.className = 'news';
	// elmDiv.appendChild(elmDl);
		if (item.comment.length > 0) {

		var text = item.comment;
		elmLi3.innerHTML = text;
		elmUl.appendChild(elmLi3);
	}
	...
	if (item.link.length > 0) {
		...
		elmLi.appendChild(document.createTextNode( zen2han(item.title) ));

		// <dd>要素に<a>要素を追加する
		elmLi2.appendChild(elmA);
	} else {
