...
//------------------------------
function drawDataArea(data, year, tabm) {
	...
	if (true == elmDiv.hasChildNodes()) {
		...
		for (i = 0; i < elmDiv.childNodes.length; i++) {
			if (null != elmDiv.childNodes[i].nodeName && undefined != elmDiv.childNodes[i].nodeName) {
				if (elmDiv.childNodes[i].nodeName.toUpperCase() == 'div'.toUpperCase()) {
...
//			total += 1;
	...
	if (total == 0) {
		...
		elmDl.className = 'newsBox shadow deepBlue bgWhite'
...
function createData(elmDiv, item, tab) {
	...
	switch(tab){		
		...
		case "release":
			if(item.news_type != "press" && item.news_type != "yuho" && item.news_type != "tanshin"){
				return;
			}
			break;
	...
	element.className = 'newsBox shadow deepBlue bgWhite'
	elmDiv.appendChild(element);
	
	// <dt>要素を新規に作成する
	var elmDt = document.createElement('h4');
		elmDt.className = 'newsTitle bgHighBlue white';
	// 作成した<dt>要素に各表示項目の「format_date」の値を追加する
		...
		elmTime.className = 'newsTime right black';
	...
	// <dd>要素を新規に作成する
		...
		var newsUl = document.createElement('p');
			newsUl.className = 'fontSize18 normal black';
...
function selIndex(index) {
	...
	// 表示データの更新を行う
	drawDataArea(org_data, year, tab);
}
...
function drawMenuArea(data) {
	...
	// イベントハンドラを追加する
	addEventListener(elmA, 'click', function(){ return YearLink_onclick(0);return false; });
	// Textの設定
...
function CreateAnchorElm(year) {
	...
	// イベントハンドラを追加する
	addEventListener(elmAYear, 'click', function(){ return YearLink_onclick(year);return false; });
	// Textの設定
...
function TabLink_onclick(tabm,n) {
	...
	// 表示データの更新を行う
	drawDataArea(org_data, yeart, tabm);
